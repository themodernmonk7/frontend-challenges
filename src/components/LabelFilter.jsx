...
const LabelFilter = () => {
  ...
  const [filterAnimalClass, setFilterAnimalClass] = useState([])
  const animalClasses = Array.from(
    new Set(animalData.map((animal) => animal.class))
  )

  const handleFilterAnimalName = () => {
    const result = animalData.filter((animal) =>
      filterAnimalClass.includes(animal.class)
    )
    setFilterAnimalName(result)
  }

  useEffect(() => {
    handleFilterAnimalName()
  }, [filterAnimalClass])

  console.log(filterAnimalClass)
  console.log(filterAnimalName)
    ...
    <Wrapper>
      <div data-testid="labels-wrapper-id" className="label-container">
        {animalClasses.map((animalClass) => (
          <div
            onClick={() =>
              setFilterAnimalClass((prevState) =>
                prevState.includes(animalClass)
                  ? prevState.filter((item) => item !== animalClass)
                  : [...prevState, animalClass]
              )
            }
            data-testid="label-id"
            className={`label ${
              filterAnimalClass.includes(animalClass) ? "active" : ""
            }`}
            key={animalClass}
          >
            {animalClass}
      ...
      <div data-testid="tile-container-id" className="tile-container">
        {(filterAnimalClass.length > 0 ? filterAnimalName : animalData).map(
          (animal) => (
            <div
